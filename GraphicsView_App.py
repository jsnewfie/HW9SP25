...
class RigidLink(qtw.QGraphicsItem):
    ...
    def boundingRect(self): #Returns the transformed bounding rectangle of the item, used for redraw and hit-testing.
        boundingRect = self.transform.mapRect(self.rect)
        return boundingRect

    def deltaY(self): #Computes and returns the vertical distance between start and end points.
        self.DY=self.endY-self.startY
        return self.DY

    def deltaX(self): #Computes and returns the horizontal distance between start and end points.
        self.DX=self.endX-self.startX
        return self.DX

    def linkLength(self): #Computes and returns the Euclidean distance (length) between the start and end points.
        self.length = math.sqrt(math.pow(self.deltaX(), 2) + math.pow(self.deltaY(), 2))
        return self.length

    def linkAngle(self): #Calculates the angle (in radians) of the link with respect to the horizontal axis. Handles vertical lines as a special case.
        self.linkLength()
        if self.length == 0.0:
            self.angle=0
        else:
...
class RigidPivotPoint(qtw.QGraphicsItem):
    ...
    def boundingRect(self): #Returns the transformed bounding rectangle of the pivot.
        bounding_rect = self.transformation.mapRect(self.rect)
        return bounding_rect

    def rotate(self, angle): #Sets the rotation angle of the pivot for the transform.
        self.rotationAngle=angle

    def paint(self, painter, option, widget=None):
        """
        Draws the pivot structure:
        - Sloped lines forming a support.
        - An arc (the pivot point).
        - Base and crosshatch pattern representing ground.
...
class MainWindow(Ui_Form, qtw.QWidget):
    ...
    def setupPensAndBrushes(self):
        """
        Creates reusable QPen and QBrush objects with various colors, patterns, and widths
        for drawing links, pivots, and background elements.
        """
        #make the pens first
        #a thick green pen
    ...
    def mouseMoveEvent(self, a0: qtg.QMouseEvent):
        """
        Handles mouse movement over the main window.
        Updates the window title with the current cursor position and hovered widget name.
        """
    ...
    def eventFilter(self, obj, event):
        """
        Captures and processes mouse events from the QGraphicsScene.
        Tracks cursor position and computes angles of rigid links based on mouse movement.
        Enables interactivity with the mechanical elements in the scene.
        """
        if obj == self.scene:
            et=event.type()
